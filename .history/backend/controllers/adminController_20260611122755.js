    ...
    router.get("/users/:id/tasks",authMiddleware,adminMiddleware,async(req,res)=>{
        try{
            ...
            const foundUser=await user.findById(userId)
            if(!foundUser) return res.status(404).json({message:"user not found"})
                res.status(200).json({tasks:foundUser.tasks})
        }
        catch(error){
            res.status(500).json({message:"internal server error"})
        }
    })
    router.get("/users/:id/tasks/deleted",authMiddleware,adminMiddleware,async(req,res)=>{
        try{
            const userId=req.params.id
            const foundUser=await user.findById(userId)
            if(!foundUser) return res.status(404).json({message:"user not found"})
    ...
    router.get("/tasks",authMiddleware,adminMiddleware,async(req,res)=>{
        try{
            const tasks=await user.aggregate([
                {$unwind:"$tasks"},
                {$project:{_id:0,username:1,"tasks.title":1,"tasks.description":1}}
            ])
